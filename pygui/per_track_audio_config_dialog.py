...
class PerTrackAudioConfigDialog(QDialog):
    ...
    def __init__(self, audio_files, default_options=None, parent=None, segment_origins=None):
        ...
        # These filters can damage segment boundaries during split operations
        if self.segment_origins:
            debug_msg = f'[SPLIT_AUDIO_DETECTED] {len(self.segment_origins)} segment(s). Disabling trim/silence_trim/fade by default.'
            if hasattr(self.parent(), 'logger'):
                self.parent().logger.log(debug_msg)
            else:
                print(f'[PER_TRACK_CONFIG] {debug_msg}')
            
            self.default_options['trim_enabled'] = False  # Audio Trimming off for splits
            self.default_options['silence_trim_enabled'] = False  # Silence Trimming off - too aggressive on segment edges
            self.default_options['fade_enabled'] = False  # Fade In/Out off - segments already have clean boundaries
        else:
            if hasattr(self.parent(), 'logger'):
    ...
    def _get_audio_duration(self, file_path: str) -> float:
        ...
        Used internally to convert "fade out in last X seconds" to absolute position.
        ...
        try:
            ...
            # FFmpeg outputs duration in stderr like: Duration: HH:MM:SS.ms
            for line in result.stderr.split('\n'):
                if 'Duration:' in line:
                    # Extract duration string like "00:05:30.50"
        ...
        for tool_name, controls in self.audio_controls.items():
            ...
            for control_key, control_widget in controls.items():
                ...
                if tool_name == 'silence_trim' and control_key == 'trim_start_cb':
                    ...
                    # Special handling: Convert "seconds from end" to "seconds from start"
                    try:
                        fade_out_from_end_str = control_widget.text()
                        fade_out_from_end_sec = parse_time_string(fade_out_from_end_str)
                        
                        # Get track duration to calculate absolute start time
                        track_duration = self._get_audio_duration(current_file_path)
                        
                        # Calculate fade_out_start (absolute position in track)
                        if track_duration > 0 and fade_out_from_end_sec >= 0:
                            fade_out_start_sec = max(0, track_duration - fade_out_from_end_sec)
                            settings['fade_out_start'] = fade_out_start_sec
                        else:
                            # Fallback: just store the value
                            settings['fade_out_start'] = fade_out_from_end_str
                        
                        # Also store the duration portion
                        settings['fade_out_duration'] = fade_out_from_end_str
                    except Exception as e:
                        print(f"[FADE_SAVE] Error: {e}")
                        settings['fade_out_duration'] = control_widget.text()
        ...
        for tool_name, controls in self.audio_controls.items():
            ...
            for control_key, control_widget in controls.items():
                ...
                if tool_name == 'silence_trim' and control_key == 'trim_start_cb':
                    ...
                    # Special handling: Convert "seconds from start" back to "seconds from end" for display
                    try:
                        fade_out_start_value = settings.get('fade_out_start')
                        
                        if fade_out_start_value is not None:
                            # Convert numeric or string value to float seconds
                            if isinstance(fade_out_start_value, str):
                                fade_out_start_sec = parse_time_string(fade_out_start_value)
                            else:
                                fade_out_start_sec = float(fade_out_start_value)
                            
                            # Get track duration
                            track_duration = self._get_audio_duration(file_path)
                            
                            # Calculate seconds from end
                            if track_duration > 0:
                                fade_out_from_end_sec = max(0, track_duration - fade_out_start_sec)
                                # Convert back to time string format (e.g., "0hr0m5s")
                                hours = int(fade_out_from_end_sec // 3600)
                                remaining = fade_out_from_end_sec % 3600
                                minutes = int(remaining // 60)
                                seconds = int(remaining % 60)
                                fade_out_from_end_str = f"0hr{minutes}m{seconds}s"
                                control_widget.setText(fade_out_from_end_str)
                            else:
                                control_widget.setText(control_widget.placeholderText())
                        else:
                            control_widget.setText(control_widget.placeholderText())
                    except Exception as e:
                        print(f"[FADE_LOAD] Error: {e}")
                        control_widget.setText(control_widget.placeholderText())
