...
from PyQt5.QtWidgets import (
    ...
    QWidget, QFileDialog, QMessageBox, QScrollArea, QFrame, QMenu
)
...
class ReplaceTracksDialog(QDialog):
    ...
    def __init__(self, parent=None, logger=None, patch_mode='replace', mod_name='', existing_selections=None):
        ...
        # Store logger for use throughout the dialog
        self.logger = logger
    ...
    def _build_biome_selection_panel(self) -> QWidget:
        ...
        # Add Clear All button (always visible for starting fresh) [per UI_STYLE_GUIDE.md]
        clear_all_layout = QHBoxLayout()
        clear_all_layout.addStretch()
        clear_all_btn = QPushButton('Clear All Selections')
        ...
        clear_all_btn.setToolTip('Start fresh - remove all biome and track selections')
        clear_all_btn.clicked.connect(self._clear_all_selections)
        clear_all_layout.addWidget(clear_all_btn)
        ...
        if self.replace_selections:
            ...
            for (category, biome_name) in sorted(self.replace_selections.keys()):
                ...
                # Add delete button [per UI_STYLE_GUIDE.md - Button Danger]
        ...
        for category, biome_name in biomes:
            ...
            # Skip biomes with no tracks - nothing to replace in Replace mode
    ...
    def show_help(self):
        """Open the help dialog showing biome selection information."""
        try:
            help_dialog = HelpWindow(self, 'biome_selection')
            help_dialog.exec_()
    ...
    def _clear_all_selections(self):
        """Clear all biome and track selections with user confirmation and feedback"""
        if not self.replace_selections:
            QMessageBox.information(self, 'Already Clear', 'There are no selections to clear.')
            return
        
        reply = QMessageBox.question(
            self,
            'Clear All Selections?',
            'This will remove all biome and track selections. Continue?',
        ...
        if reply == QMessageBox.Yes:
            ...
            if self.logger:
                self.logger.log('[Replace] User cleared all selections via "Clear All Selections" button', context='ReplaceDialog')
    ...
    def _remove_biome_selection(self, biome):
        ...
        if biome in self.replace_selections:
            ...
            if self.logger:
                self.logger.log(f'[Replace] Removed biome selection: {category}: {biome_name}', context='ReplaceDialog')
    ...
    def _show_track_selection_panel(self):
        ...
        # Apply dark palette following UI Style Guide
        palette = QPalette()
        palette.setColor(QPalette.Window, QColor('#1a1f2e'))  # Main background
        palette.setColor(QPalette.WindowText, QColor('#e6ecff'))  # Primary text
        palette.setColor(QPalette.Base, QColor('#283046'))  # Input field background
        palette.setColor(QPalette.AlternateBase, QColor('#2a2a3a'))
        palette.setColor(QPalette.Text, QColor('#e6ecff'))  # Primary text
        palette.setColor(QPalette.Button, QColor('#2a3a4a'))  # Neutral button
        palette.setColor(QPalette.ButtonText, QColor('#e6ecff'))  # Button text
        dlg.setPalette(palette)
        
        layout = QVBoxLayout(dlg)
        
        # Title - Following UI Style Guide (14px, bold, cyan)
        ...
        # Subtitle/hint - Following UI Style Guide (11px, italic, gray)
        info = QLabel(
        ...
        info.setStyleSheet('color: #888888; font-size: 11px; font-style: italic; margin-bottom: 12px;')
        info.setWordWrap(True)
        layout.addWidget(info)
        ...
        if not day_tracks and not night_tracks:
            ...
            preview_info.setStyleSheet('color: #6fff99; background-color: #1a3a1a; padding: 8px; border-radius: 4px; font-size: 10px; font-weight: bold;')
            ...
            # Day tracks
            if day_tracks:
                # Day label with Clear All button - Following UI Style Guide
                day_header = QHBoxLayout()
                day_label = QLabel('Day Tracks')
                day_label.setStyleSheet('color: #ffcc00; font-size: 11px; font-weight: bold; margin-top: 8px;')
                ...
                clear_all_day_btn.setStyleSheet('background-color: #c41e3a; color: #e6ecff; font-size: 10px; border-radius: 4px; padding: 4px 8px;')
            ...
            # Night tracks
            if night_tracks:
                # Night label with Clear All button - Following UI Style Guide
                night_header = QHBoxLayout()
                night_label = QLabel('Night Tracks')
                night_label.setStyleSheet('color: #ffcc00; font-size: 11px; font-weight: bold; margin-top: 12px;')
                ...
                clear_all_night_btn.setStyleSheet('background-color: #c41e3a; color: #e6ecff; font-size: 10px; border-radius: 4px; padding: 4px 8px;')
        ...
        cancel_btn.setStyleSheet('background-color: #c41e3a; color: #e6ecff; border-radius: 4px; padding: 8px 16px; font-weight: bold; min-width: 100px;')
        cancel_btn.clicked.connect(dlg.reject)
        btn_row.addWidget(cancel_btn)
        
        done_btn = QPushButton('Done - Show Summary')
        done_btn.setStyleSheet('background-color: #2d5a3d; color: #e6ecff; border-radius: 4px; padding: 8px 16px; font-weight: bold; min-width: 120px;')
        ...
        help_btn.setToolTip('Show help for biome selection')
        help_btn.setStyleSheet('background-color: #2a3a4a; color: #e6ecff; border: 1px solid #3a4a6a; border-radius: 4px; padding: 6px; font-weight: bold; font-size: 14px;')
    ...
    def _get_vanilla_track_path(self, category: str, biome: str, day_or_night: str, index: int) -> Path:
        """Get the path to a vanilla track file for preview playback"""
        from utils.patch_generator import get_vanilla_tracks_for_biome
        from pathlib import Path
        ...
        try:
            ...
            if 0 <= index < len(tracks):
                track_filename = tracks[index]
                
                # Build full path to vanilla track file
                # vanilla_tracks/{category}/{biome}/{day|night}/{filename}
                module_dir = Path(__file__).parent.parent  # Go from dialogs/ to pygui/
                track_path = module_dir / 'vanilla_tracks' / category / biome / day_or_night.lower() / track_filename
    ...
    def _on_assign_track_file(self, index: int, day_or_night: str, category: str, biome: str, btn: QPushButton, label: QLabel, status_label: QLabel = None):
        ...
        if self.mod_name:
            try:
                from pathlib import Path
                # Path: pygui/dialogs/replace_tracks_dialog.py → go up 3 levels to StarSound root
                mod_music_folder = Path(__file__).parent.parent.parent / 'staging' / self.mod_name / 'music'
    ...
    def _show_summary_dialog(self):
        ...
        for (category, biome), tracks_dict in self.replace_selections.items():
            ...
            # Get vanilla tracks for this biome to show replace mapping
            from utils.patch_generator import get_vanilla_tracks_for_biome
            vanilla_data = get_vanilla_tracks_for_biome(category, biome)
            ...
            if tracks_dict.get('day'):
                ...
                for index in sorted(tracks_dict['day'].keys()):
                    ...
                    custom_filename = Path(ogg_path).name
                    
                    # Get vanilla track name if available
                    if 0 <= index < len(day_vanilla):
                        vanilla_filename = day_vanilla[index]
                        display_text = f'    {vanilla_filename} ---> {custom_filename}'
                    else:
                        display_text = f'    Track {index}: ---> {custom_filename}'
            ...
            if tracks_dict.get('night'):
                ...
                for index in sorted(tracks_dict['night'].keys()):
                    ...
                    # Get vanilla track name if available
                    if 0 <= index < len(night_vanilla):
                        vanilla_filename = night_vanilla[index]
                        display_text = f'    {vanilla_filename} ---> {custom_filename}'
                    else:
                        display_text = f'    Track {index}: ---> {custom_filename}'
