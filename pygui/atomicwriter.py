...
def backup_and_convert_audio(file_path: str, mod_folder: str, bitrate: str = '192k', audio_filter: str = '', ffmpeg_log_callback=None) -> tuple:
    """
    ...
    Args:
        ...
        mod_folder: Destination mod folder
    ...
    logger = get_logger()
    logger.log(f'[DEBUG] backup_and_convert_audio: mod_folder={mod_folder}')
...
def create_mod_folder_structure(final_destination: Path, mod_name: str) -> Path:
    ...
    # Clean up any misplaced .ogg files in originals folder
    try:
        logger = get_logger()
        originals_folder = mod_folder / 'backups' / 'originals'
        if originals_folder.exists():
            for file in originals_folder.glob('*.ogg'):
                try:
                    file.unlink()
                    logger.log(f'[CLEANUP] Removed misplaced OGG from originals: {file.name}')
                except Exception as e:
                    logger.warn(f'[CLEANUP] Could not remove misplaced OGG: {file.name} ({e})')
    except Exception as e:
        print(f'[CLEANUP] Error cleaning up originals folder: {e}')
