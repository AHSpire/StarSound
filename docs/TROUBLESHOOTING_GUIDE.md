...
| **Format not supported** | StarSound supports: **MP3, MP4, FLAC, WAV, OGG** <br> If using other format, convert via: [Audacity](https://www.audacityteam.org/) or [FFmpeg](https://ffmpeg.org/) |
...
| **Only vanilla music playing** | 1. Check mod is enabled: Starbound mods list <br> 2. If using "Remove Mode" (recommended): Only your custom music should play <br> 3. If using "Add Mode": Vanilla + custom should mix, check file count <br> 4. If using "Replace Mode": Only custom should play, verify track swaps <br> 5. Disable conflicting mods (other music mods) <br> 6. Restart game completely |
